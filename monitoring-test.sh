...
if [ ! -f "$LOG_FILE" ]; then
    touch "$LOG_FILE" 2>/dev/null || {
        echo "$(date): ERROR — cannot create $LOG_FILE" >&2
        exit 1
    }
    chmod 644 "$LOG_FILE"
fi
...
if [ -n "$CURRENT_PID" ]; then
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if [ "$CURRENT_PID" != "$LAST_PID" ]; then
            echo "$(date): Process '$PROCESS_NAME' DEAD (PID: $LAST_PID)" >> "$LOG_FILE"
            echo "$(date): Process '$PROCESS_NAME' STARTED (PID: $CURRENT_PID)" >> "$LOG_FILE"
        fi
    else
        echo "$(date): Process '$PROCESS_NAME' STARTED (PID: $CURRENT_PID)" >> "$LOG_FILE"
    fi
    ...
    if ! curl -fs --max-time 10 "$URL" > /dev/null; then
        echo "$(date): ERROR — HTTPS endpoint '$URL' is unavailable!" >> "$LOG_FILE"
    fi
